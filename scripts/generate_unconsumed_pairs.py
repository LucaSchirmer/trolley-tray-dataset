...
image with the following shape:
    ...
    {
    "source_dir": "images_cropped/unconsumed/salad",
      "unconsumed": "images_cropped/unconsumed/.../image.jpg",
      "consumed": [],
      "possibleElements": []
    }
...
`images_cropped/unconsumed`), it can also create one JSON file per populated
subdirectory.
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--input-dir",
        default="images_cropped/unconsumed",
    ...
    parser.add_argument(
        "--base-url",
        default="",
        help="Optional base URL used to convert unconsumed paths into absolute URLs.",
    )
    parser.add_argument(
        "--batch",
        action="store_true",
        ...
        for target_dir in targets:
            ...
            if args.base_url:
                for entry in entries:
                    ...
                    if isinstance(unconsumed, str) and unconsumed and not unconsumed.startswith("http"):
                        entry["unconsumed"] = f"{args.base_url}/{unconsumed}"
